...
def Searching():
    ...
    if OP == 1:
        ...
        Items = list(map(int, input("Enter the item you want to count: ").split()))
        ...
        Items = list(map(int, input("Enter the items you want to count: ").split()))
...
def ShellSort(Order):
    ...
    while gap > 0:
        for i in range(gap, N):
            ...
            j = i
            while j >= gap and (
                (Order == 1 and A[j - gap] > temp) or (Order == 2 and A[j - gap] < temp)
            ):
                A[j] = A[j - gap]
                j -= gap
            A[j] = temp
