...
class Team(SQLModel, table=True):
    ...
    name: str
    headquarters: str
    heros: List["TeamHero"] = Relationship(back_populates="team")
...
def create_heroes():
    with Session(engine) as session:
        team_xman = Team(id = 3 , name="X-MEN", headquarters="X Tower")
        team_titans = Team( id = 4 , name="Team Titans", headquarters="Titans Place")
        # session.add(team_avengers)
        # session.add(team_justice_league)
        # # session.commit()

        hero_1 = TeamHero(id = 4,name="wolverine" , secret_name = "Logan" , age = 45 , team = team_xman)
        hero_2 = TeamHero(id = 6,name="flash" , secret_name = "Barry Allen" , age = 28 , team = team_titans)
        session.add_all([team_xman,team_titans,hero_1,hero_2])
...
def create_teams_and_heroes():
    with Session(engine) as session:
        hero_black_widow = TeamHero(name="Black Widow", secret_name="Natasha Romanoff", age=35)
        hero_black_panther = TeamHero(name="Black Panther", secret_name="T'Challa", age=35)
        team = Team(name="DarthVaders", headquarters="Galaxy Station", heros=[hero_black_widow, hero_black_panther])
        session.add(team)
        session.commit()
        session.close()
